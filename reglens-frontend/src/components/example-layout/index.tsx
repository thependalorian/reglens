...
export function ExampleLayout({ chatContent, appContent }: ExampleLayoutProps) {
  // Default to the split view: the canvas (analysis + documents) is the
  // point of RegLens, so it is visible alongside the chat from first load.
  const [mode, setMode] = useState<"chat" | "app">("app");

  useFrontendTool({
    name: "enableAppMode",
    description:
      "Open the analysis canvas (findings, coverage, report, and documents).",
    handler: async () => {
      setMode("app");
    },
  });
  ...
  return (
    <div className="h-full flex flex-row pb-6">
      ...
      {/* Chat Content */}
      <div
        className={`max-h-full flex flex-col bg-card ${
          mode === "app"
      ...
      >
        ...
        <div className="shrink-0 pt-[23px] pl-6 pb-2 max-lg:pb-4 flex gap-2.5 items-baseline align-center">
          <span className="flex items-center gap-2">
            <span
              aria-hidden
              className="inline-block h-4 w-1 rounded-full bg-burgundy"
            />
            <span className="font-display text-[1.65rem] leading-none tracking-tight text-ink">
              RegLens
            </span>
          </span>
          <span className="text-xs text-ink-muted max-sm:hidden">
            Regulatory sludge intelligence
          </span>
        </div>
